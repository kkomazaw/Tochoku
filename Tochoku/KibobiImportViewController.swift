//
//  KibobiImportViewController.swift
//  Tochoku
//
//  Created by Matsui Keiji on 2018/10/14.
...
class KibobiImportViewController: UIViewController {
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        if kibobiText.text.range(of: "夕診") == nil{
            if let index = arr.firstIndex(where:{$0.contains("日直不可日")}){
                arr.insert("指定なし", at: index)
                arr.insert("夕診回数", at: index)
            }
            if let index = arr.firstIndex(where:{$0.contains("日直希望日")}){
        ...
        let nicchokufukabiIndex = arr.firstIndex(where:{$0.contains("日直不可日")}) ?? yushinkaisuIndex + 2
        ...
        let nicchokukibobiIndex = arr.firstIndex(where:{$0.contains("日直希望日")}) ?? yushinfukabiIndex + 2
        ...
        print(nicchokufukabiIndex)
        print(tochokufukabiIndex)
        print(yushinfukabiIndex)
        print(nicchokukibobiIndex)
        ...
        for i in yushinfukabiIndex + 1 ..< nicchokukibobiIndex{
            if let _ = selectedDateFormatter.date(from: arr[i]){
